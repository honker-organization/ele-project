...
export default {
  ...
  data() {
    return {
      ...
      active: 2,
      tab:['全部','面食粥点','简餐便当','汉堡披萨','香锅冒菜','小吃炸串','地方菜系','日韩料理','轻食简餐']
    };
  },
  components:{
  [Tab.name]:Tab,
    ...
    sortArr(type){
      // const {shopList} = this
       ...
       switch(type){
        ...
        case 2: //起送价最低
            return newShopList.sort((a,b) => a.piecewise_agent_fee.rules[0].price - b.piecewise_agent_fee.rules[0].price)

        case 3: //配送最快
            return newShopList.sort((a,b) => a.order_lead_time - b.order_lead_time )
       
        case 4: //配送费最低
            return newShopList.sort((a,b) => a.piecewise_agent_fee.rules[0].price - b.piecewise_agent_fee.rules[0].price)
       
       case 5: //人均从低到高
            return newShopList.sort((a,b) => a.piecewise_agent_fee.rules[0].fee - b.piecewise_agent_fee.rules[0].fee)
       
       case 6: //人均从高到底
            return newShopList.sort((a,b) => b.piecewise_agent_fee.rules[0].fee - a.piecewise_agent_fee.rules[0].fee) 
       
       case 7: //通用排序
          return newShopList.sort((a,b)=> b.recent_order_num - a.recent_order_num)
  ...
  computed:{
    ...mapState({shopList:state =>state.shop.shopList}),
    sortShoplist(){
      const {value} = this
     console.log(this.shopList)
       // 根据value值进行排序
